...
# 2.边缘检测+二值化
edge = Canny(gray)
edge[0:10, 0:edge.shape[1]] = 0
edge[edge.shape[0]-10:edge.shape[0], 0:edge.shape[1]] = 0
edge[0:edge.shape[0], 0:10] = 0
edge[0:edge.shape[0], edge.shape[1]-10:edge.shape[1]] = 0
...
# [::-1]   反序->从大到小
# [:15]    前15个
ind_x = np.argsort(hough.ravel())[::-1][:15]
ind_y = ind_x.copy()
thetas = ind_x % 180
rhos = ind_y // 180
